...
def build_mapping_payload(
    ...
    """Build mapping asset payload from YAML metadata."""
    asset_name = yaml_data.get("name") or yaml_file.stem
    mapping_type = yaml_data.get("mapping_type")
...
def fetch_mapping_assets(client: httpx.Client, assets_url: str) -> dict[str, list[dict[str, Any]]]:
    ...
    """
    response = client.get(assets_url, params={"asset_type": "mapping"})
...
def execute_import(
    ...
    for yaml_file in files:
        ...
        payload = build_mapping_payload(yaml_file, yaml_data, actor)
        asset_name = payload["name"]

        print(f"\n=== {yaml_file.relative_to(REPO_ROOT)} ===")
        print(f"name: {asset_name}")
        print(f"mapping_type: {payload['mapping_type']}")
        print(f"scope: {payload['scope']}")
        print(f"description: {payload['description'][:60]}...")
        print(f"views: {list(payload['content'].get('views', {}).keys())}")
        ...
        if publish and asset_id:
            ...
            publish_payload = {"published_by": actor or "mapping_migration"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import Mapping Seeds into the Asset Registry.")
    parser.add_argument(
        "--base-url",
