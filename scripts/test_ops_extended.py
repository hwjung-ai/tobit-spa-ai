...
TEST_CASES = [
    ...
    {"id": 10, "query": "How many audit log entries are there?", "expected": "743", "category": "count"},
    ...
    {"id": 37, "query": "How many documents are successfully processed?", "expected": "79", "category": "count"},
...
def check_answer(response_text: str, expected: str) -> tuple[bool, str]:
    ...
    if expected.isdigit():
        try:
            ...
            if formatted in response_text:
                return True, f"Found formatted: {formatted}"
        except Exception:
            pass

    # 숫자 추출 시도 (쉼표 포함 숫자도 포함)
    ...
    if expected_lower == "no":
        neg_patterns = [
            "없습니다", "없어", "없음", "0건", "0개", "발견되지 않", "존재하지 않",
            "not found", "no inactive", "none", "0 items", "found 0",
            "발견되지 않았습니다", "확인되지 않습니다", "없는 것으로",
            "발견되지 않", "존재하지 않", "없는 것", "없다",
            "inactive", "failed", "실패한", "처리에 실패",
            "0건으로", "0개로", "0 건", "0 개",
            "total results: 0", "total results is 0",
            "발견되지 않은", "없는 것으로 보입니다", "없는 것으로 확인",
            "no documents", "no items", "no results",
            "0개입니다", "0건입니다",
        ]
        if any(p in response_lower for p in neg_patterns):
            return True, f"Found negation for 'no'"
        # 숫자 0이 포함된 경우도 "no"로 처리
        zero_patterns = re.findall(r'\b0\b', response_text)
        if zero_patterns:
            return True, f"Found 0 count for 'no'"

    # 소수점 숫자 확인 (예: 74.9%)
    if "." in expected:
        decimal_nums = re.findall(r'[\d]+\.[\d]+', response_text)
        for num in decimal_nums:
            if num == expected:
                return True, f"Found decimal: {num}"
        # 반올림 허용 (±0.5)
        try:
            exp_val = float(expected)
            for num in decimal_nums:
                if abs(float(num) - exp_val) < 0.6:
                    return True, f"Found approx decimal: {num}"
        except ValueError:
            pass

    # 한국어 숫자 표현 확인 (예: "280개", "280건")
    if expected.isdigit():
        korean_patterns = [
            f"{expected}개", f"{expected}건", f"{expected}개의", f"{expected}건의",
            f"{expected}개가", f"{expected}건이", f"{expected}개로", f"{expected}건으로",
            f"{expected}개입니다", f"{expected}건입니다",
        ]
        if any(p in response_text for p in korean_patterns):
            return True, f"Found Korean number: {expected}"
        # 쉼표 형태 + 한국어
        if formatted:
            korean_formatted = [
                f"{formatted}개", f"{formatted}건", f"{formatted}개의",
                f"{formatted}개입니다", f"{formatted}건입니다",
            ]
            if any(p in response_text for p in korean_formatted):
                return True, f"Found Korean formatted: {formatted}"

    # AGGREGATE Total 값 확인 (응답에 "Total: N" 또는 "AGGREGATE Total: N" 형태)
    if expected.isdigit():
        agg_patterns = [
            rf'total[:\s]+{re.escape(expected)}',
            rf'aggregate[:\s]+{re.escape(expected)}',
            rf'합계[:\s]*{re.escape(expected)}',
            rf'총[:\s]*{re.escape(expected)}',
            rf'count[:\s]+{re.escape(expected)}',
        ]
        for pat in agg_patterns:
            if re.search(pat, response_lower):
                return True, f"Found aggregate total: {expected}"

    # 숫자가 볼드/마크다운 형태로 포함된 경우 (예: **280**, `280`)
    if expected.isdigit():
        bold_patterns = [
            f"**{expected}**",
            f"`{expected}`",
            f"**{formatted}**" if formatted else None,
        ]
        for pat in bold_patterns:
            if pat and pat in response_text:
                return True, f"Found bold/code number: {expected}"
        ...
        if response.status_code == 200:
            ...
            if "data" in data:
                ...
                if "blocks" in inner_data:
                    ...
                    response_text = json.dumps(inner_data, ensure_ascii=False)
            else:
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        # JWT 로그인
        login_resp = await client.post(LOGIN_URL, json=LOGIN_PAYLOAD, timeout=10.0)
        ...
        for test_case in TEST_CASES:
            ...
            if result["success"]:
                ...
                failed += 1
                print(f"  ❌ {result['reason']} ({result['response_time_ms']}ms)")
                print(f"  응답: {result['response_text'][:120]}")
            print()
