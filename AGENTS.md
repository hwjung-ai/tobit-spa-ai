...
3. **보안**: DB, Redis, Neo4j 등 외부 서비스 접속 정보는 환경 변수로만 관리하며, 코드에 하드코딩하지 않습니다.
4. **버전 관리**: 큰 규모의 변경 전후에는 `checkpoint` 커밋을 통해 작업 단계를 명확히 남깁니다.
5. **경로 명확성**: 파일 경로는 프로젝트 루트 기준 절대 경로를 사용합니다. 문맥상 명확한 경우에만 제한적으로 상대 경로를 사용합니다.
...
AI 에이전트는 모든 작업을 종료하기 전, 다음 세 가지 기준을 충족했는지 스스로 확인해야 합니다.
