...
|---------|------------|-----------|------|
| 전체 (기본) | `all` | `POST /ops/ask` | 모든 모드 통합 오케스트레이션 |
...
프론트엔드 기본 모드는 `all`(전체)이며, 전체 모드만 `/ops/ask` 엔드포인트를 사용합니다. 나머지 5개 모드는 `/ops/query`에 `mode` 파라미터로 분기합니다.
...
| 엔드포인트 | 용도 |
|-----------|------|
| `POST /ops/ask` | CI Orchestrator 통합 질의 |
