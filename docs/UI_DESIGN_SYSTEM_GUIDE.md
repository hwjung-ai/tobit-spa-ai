...
### 8.8 헤더 메인 메뉴 (Top Nav)

- 전역 헤더 메뉴(`Chat/Docs/SIM/Ops/API/Pages/CEP/Admin`)는 별도 탭 클래스를 사용합니다.
  - 공통 클래스: `main-menu-tab*`
- 활성 탭은 밑줄(underline) + 전경색으로 표시합니다.
- 비활성 탭도 hover 시 밑줄이 반드시 표시되어야 합니다.
- hover/active 모두 라이트/다크에서 대비가 유지되어야 합니다.
- 페이지별 개별 클래스 재정의 대신 공통 클래스(`globals.css`)를 우선 사용합니다.
