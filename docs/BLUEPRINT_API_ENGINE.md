...
- ❌ **API 생성**
  - API 타입 선택 (SQL, HTTP, Python, Workflow)
  - API 이름 및 설명 입력
  - 파라미터 스키마 정의
  - 런타임 정책 설정

- ❌ **API 편집**
  - SQL 에디터 (syntax highlighting)
  - HTTP 빌더 (Form + JSON 모드)
  - Python 에디터 (syntax highlighting)
  - Workflow 빌더 (시각적 노드 에디터)
...
### 4. API Test Runner (미구현)

#### 4.1 필요한 기능

**경로:** `/admin/api-test` (미구현)
...
| **API Manager UI** | ⭐⭐ | 컴포넌트만 완료, 페이지 미구현 |
| **API Builder UI** | ⭐ | 미구현 |
| **API Test Runner** | ⭐ | 미구현 |
| **전체** | ⭐⭐⭐ | 70% 완료 |
