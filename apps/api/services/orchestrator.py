...
class OpenAIOrchestrator(BaseOrchestrator):
            ...
            async for event in self._llm.stream_response(
                ...
                **self._stream_kwargs(),
                ...
                if event_type == "response.created":
                    # Response started
                    logging.debug(
                        "OpenAI Response created: %s", event.get("response_id")
                    )
    ...
    async def stream_chat(
        ...
        try:
            ...
            yield {
                "type": "summary",
                "text": "Generated by OpenAI stream (Responses API)",
            }
            yield {"type": "done", "text": "OpenAI stream complete"}
        except Exception as exc:
            logging.exception(
                "OpenAI stream failed; falling back to synchronous completion: %s", exc
            )
            ...
            if fallback_text:
                ...
                yield {
                    "type": "summary",
                    "text": "Fallback summary; streaming not available",
                }
                yield {"type": "done", "text": "Fallback complete"}
                return
            logging.warning(
                "Fallback synchronous completion failed; using fake orchestrator"
            )
