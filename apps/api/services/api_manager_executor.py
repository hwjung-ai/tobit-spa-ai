...
def execute_sql_api(
    ...
    try:
        ...
        if not validation["is_safe"]:
            raise ValueError(
                f"SQL validation failed: {', '.join(validation['errors'])}"
            )
    ...
    execution_log = record_exec_log(
        ...
        response_data={"data": result_data, "rows": rows_affected}
        if result_data
        else None,
...
def execute_http_api(
            ...
            if isinstance(value, str):
                ...
                for match in matches:
                    ...
                    if param_key in params:
                        value = value.replace(
                            f"{{params.{param_key}}}", str(params[param_key])
                        )
                return value
        ...
        with httpx.Client(timeout=HTTP_TIMEOUT) as client:
            if method == "GET":
                ...
                response = client.post(
                    url, headers=headers, json=body if body else params
                )
            elif method == "PUT":
                response = client.put(
                    url, headers=headers, json=body if body else params
    ...
    execution_log = record_exec_log(
        ...
        duration_ms=duration_ms,
        request_params=params,
        response_data={"status_code": status_code, "data": result_data}
        if result_data
        else None,
        response_status="success" if error_message is None else "error",
        error_message=error_message,
...
def substitute_workflow_params(
    ...
    if isinstance(template, str):
        ...
        for match in matches:
            ...
            if node_id in context:
                node_result = context[node_id]
                if "columns" in node_result and isinstance(
                    node_result["columns"], list
                ):
                    columns_str = str(node_result["columns"])
                    template = template.replace(
                        f"{{steps.{node_id}.columns}}", columns_str
                    )
        ...
        for match in matches:
            ...
            if node_id in context:
                ...
                if "row_count" in node_result:
                    template = template.replace(
                        f"{{steps.{node_id}.row_count}}", str(node_result["row_count"])
                    )

        return template
    elif isinstance(template, dict):
        return {
            k: substitute_workflow_params(v, context, params)
...
def execute_workflow_api(
    ...
    try:
        ...
        logic = (
            workflow_api.logic
            if hasattr(workflow_api, "logic")
            else workflow_api.get("logic", "{}")
        )
        api_id = (
            str(workflow_api.api_id)
            if hasattr(workflow_api, "api_id")
            else workflow_api.get("api_id", "unknown")
        )
        ...
        for node_id in execution_order:
            ...
            if step_result["status"] == "fail":
                raise Exception(
                    f"Node {node_id} failed: {step_result['error_message']}"
                )
    ...
    execution_log = record_exec_log(
        ...
        response_data={"steps": steps, "final_output": final_output}
        if final_output
        else None,
        response_status="success" if error_message is None else "error",
        error_message=error_message,
