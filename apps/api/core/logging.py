...
_REQUEST_CONTEXT: ContextVar[dict[str, str]] = ContextVar(
    "request_context", default={"request_id": "-", "tenant_id": "-", "mode": "-"}
)
...
def get_request_context() -> dict[str, str]:
    ...
    return {"request_id": context.get("request_id", "-"), "tenant_id": context.get("tenant_id", "-"), "mode": context.get("mode", "-")}


def set_request_context(request_id: str, tenant_id: str, mode: str = "ci") -> None:
    _REQUEST_CONTEXT.set({"request_id": request_id, "tenant_id": tenant_id, "mode": mode})


def clear_request_context() -> None:
    _REQUEST_CONTEXT.set({"request_id": "-", "tenant_id": "-", "mode": "-"})
...
class StructuredFormatter(logging.Formatter):
    ...
    def __init__(self) -> None:
        fmt = "%(asctime)s %(levelname)s %(name)s request_id=%(request_id)s tenant_id=%(tenant_id)s mode=%(mode)s %(message)s"
