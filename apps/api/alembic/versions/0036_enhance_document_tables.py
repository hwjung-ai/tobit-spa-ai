...
def upgrade() -> None:
    ...
    op.add_column(
        "documents", sa.Column("format", sa.String(20), nullable=True, default="pdf")
    )
    op.add_column(
        "documents",
        sa.Column("processing_progress", sa.Integer(), nullable=True, default=0),
    )
    op.add_column(
        "documents", sa.Column("total_chunks", sa.Integer(), nullable=True, default=0)
    )
    ...
    op.add_column(
        "document_chunks",
        sa.Column("chunk_version", sa.Integer(), nullable=True, default=1),
    )
    op.add_column(
        "document_chunks",
        sa.Column("chunk_type", sa.String(50), nullable=True, default="text"),
    )
    op.add_column(
        "document_chunks", sa.Column("position_in_doc", sa.Integer(), nullable=True)
    )
    op.add_column(
        "document_chunks", sa.Column("page_number", sa.Integer(), nullable=True)
    )
    op.add_column(
        "document_chunks", sa.Column("slide_number", sa.Integer(), nullable=True)
    )
    op.add_column("document_chunks", sa.Column("table_data", JSONB(), nullable=True))
    op.add_column(
        "document_chunks", sa.Column("source_hash", sa.String(64), nullable=True)
    )
    op.add_column(
        "document_chunks", sa.Column("relevance_score", sa.Float(), nullable=True)
    )

    # Create indexes for search
    op.create_index("ix_document_chunks_chunk_type", "document_chunks", ["chunk_type"])
    op.create_index(
        "ix_document_chunks_page_number", "document_chunks", ["page_number"]
    )
