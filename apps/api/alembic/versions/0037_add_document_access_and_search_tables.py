...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('access_type', sa.String(50), nullable=False),  # read, download, share
    ...
    op.create_table(
        ...
        sa.Column('search_type', sa.String(20), nullable=False),  # text, semantic, hybrid
    ...
    op.create_index('ix_document_search_log_user_id', 'document_search_log', ['user_id'])
    op.create_index('ix_document_search_log_tenant_id', 'document_search_log', ['tenant_id'])
    op.create_index('ix_document_search_log_created_at', 'document_search_log', ['created_at'])
    ...
    op.create_table(
        ...
        sa.Column('extraction_method', sa.String(50), nullable=True),  # pdf_text, ocr, etc.
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, default=sa.func.now()),
    )

    op.create_index('ix_document_chunk_metadata_chunk_id', 'document_chunk_metadata', ['chunk_id'])
    op.create_index('ix_document_chunk_metadata_language', 'document_chunk_metadata', ['chunk_language'])
