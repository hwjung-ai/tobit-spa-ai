...
def test_ops_query_includes_trace_id(client):
    ...
    assert "x-trace-id" in response.headers, (
        "trace_id should always be present in response headers"
    )
    trace_id = response.headers.get("x-trace-id")
    assert trace_id is not None and trace_id != "", (
        "trace_id should be a non-empty value"
    )
...
def test_ops_ci_ask_includes_trace_id(client):
    ...
    trace_id = response.headers.get("x-trace-id")
    assert trace_id is not None and trace_id != "", (
        "trace_id should always be in headers"
    )
...
def test_trace_id_persists_across_requests(client):
    ...
    assert response.headers.get("x-trace-id") == custom_trace_id, (
        "Custom trace_id from header should be preserved"
    )
...
def test_parent_trace_id_in_response(client):
    ...
    assert response.headers.get("x-trace-id") == custom_trace_id
    assert response.headers.get("x-parent-trace-id") == custom_parent_trace_id, (
        "Parent trace ID from header should be preserved"
    )
