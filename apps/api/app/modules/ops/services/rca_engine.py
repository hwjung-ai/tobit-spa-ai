...
class RCAEngine:
    ...
    def analyze_single_trace(
        ...
        if (
            plan_validated
            and plan_validated.get("intent") == "list"
            and plan_validated.get("view") == "graph"
        ):
        ...
        if references:
            for ref in references:
                if (
                    ref.get("ref_type", "").startswith("SQL")
                    and ref.get("row_count") == 0
                ):
    ...
    def analyze_diff(
        ...
        if baseline_assets != candidate_assets:
            self._check_asset_changes(
                baseline_assets, candidate_assets, candidate_trace
            )
        ...
        if (
            ...
            and (
                ...
                or baseline_plan.get("output_types")
                != candidate_plan.get("output_types")
            )
        ):
        ...
        baseline_tool_names = {
            step.get("step_id", "").split(":")[0] for step in (baseline_steps or [])
        }
        candidate_tool_names = {
            step.get("step_id", "").split(":")[0] for step in (candidate_steps or [])
        }
        ...
        if (
            baseline_blocks
            and candidate_blocks
            and (
                len(baseline_blocks) != len(candidate_blocks)
                or {b.get("type") for b in baseline_blocks}
                != {b.get("type") for b in candidate_blocks}
            )
        ):
    ...
    def _check_tool_duration_spike(self, execution_steps: List[Dict]):
        ...
        durations = [
            (i, step.get("step_id", ""), step.get("duration_ms", 0))
            for i, step in enumerate(execution_steps)
        ]
    ...
    def _check_asset_changes(
        ...
        changed_assets = set(baseline_assets).symmetric_difference(
    ...
    def _check_plan_changes(self, baseline_plan: Dict, candidate_plan: Dict):
        ...
        if baseline_plan.get("intent") != candidate_plan.get("intent"):
            changes.append(
                f"intent ({baseline_plan.get('intent')} → {candidate_plan.get('intent')})"
            )
    ...
    def _check_tool_error_regression(
        self, candidate_steps: List[Dict], baseline_steps: List[Dict]
    ):
    ...
    def _check_block_structure_change(
        self, baseline_blocks: List[Dict], candidate_blocks: List[Dict]
    ):
    ...
    def _check_data_reduction(
        self, baseline_refs: List[Dict], candidate_refs: List[Dict]
    ):
        ...
        reduction_pct = (
            ((baseline_total - candidate_total) / baseline_total * 100)
            if baseline_total > 0
            else 0
        )
    ...
    def _check_performance_regression(
        self, baseline_duration: int, candidate_duration: int
    ):
        """Rule 8 (Diff): Performance regression"""
        slowdown_pct = (
            (candidate_duration - baseline_duration) / baseline_duration * 100
        )
    ...
    def _rank_hypotheses(self):
        ...
        self.hypotheses.sort(
            key=lambda h: (confidence_order.get(h.confidence, 3), h.rank)
        )
            ...
            "hypotheses": [
                {
                    ...
                    "evidence": [
                        {"path": e.path, "snippet": e.snippet, "display": e.display}
                        for e in h.evidence
                    ],
                    "checks": h.checks,
