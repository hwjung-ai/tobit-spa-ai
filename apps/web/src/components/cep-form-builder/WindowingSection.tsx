...
export function WindowingSection({
  ...
  return (
    <div className="cep-section-container">
      <h3 className="cep-section-title">윈도우 설정 (선택사항)</h3>
      ...
      <FormFieldGroup label="윈도우 타입">
        <div className="space-y-2">
          {WINDOW_TYPES.map((wt) => (
            <label
              key={wt.value}
              className="cep-condition-group flex items-start gap-3 cursor-pointer"
            >
              <input
                type="radio"
                name="windowType"
              ...
              <div className="flex-1">
                <div className="cep-condition-label">{wt.label}</div>
                <div className="cep-text-muted">{wt.description}</div>
              </div>
            </label>
          ))}
        </div>
