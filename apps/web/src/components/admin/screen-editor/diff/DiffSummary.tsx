...
export default function DiffSummary({ diff }: DiffSummaryProps) {
  ...
  return (
    <div className="px-4 py-3 bg-surface-base border-b border-variant">
      <div className="flex items-center justify-between">
        <div className="text-sm text-foreground-secondary">
          ...
          {unchanged > 0 && <span className="ml-3 text-foreground0">{unchanged} unchanged</span>}
        </div>
        {added === 0 && removed === 0 && modified === 0 && (
          <div className="text-xs text-foreground0">No changes detected</div>
        )}
      </div>
    </div>
  );
