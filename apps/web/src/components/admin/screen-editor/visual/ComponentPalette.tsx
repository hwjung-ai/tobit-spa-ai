  ...
  return (
    <div className="flex flex-col h-full" style={{ backgroundColor: "rgba(15, 23, 42, 0.5)" }}>
      ...
      <div className="border-b p-3" style={{ borderColor: "var(--border)" }}>
        <h3 className="text-sm font-semibold mb-2" style={{ color: "var(--foreground-secondary)" }}>Components</h3>
        <Input
          placeholder="Search..."
          value={searchTerm}
      ...
      {/* Target indicator */}
      {selectedContainer && (
        <div className="border-b px-3 py-2" style={{ borderColor: "var(--border)", backgroundColor: "rgba(12, 74, 110, 0.5)" }}>
          <p className="text-[10px]" style={{ color: "#bae6fd" }}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {filteredComponents.length === 0 ? (
          ...
          filteredComponents.map((desc) => (
            ...
            >
              <GripVertical className="w-3 h-3 flex-shrink-0" style={{ color: "var(--muted-foreground)" }} />
              <div className="flex-1 min-w-0">
                <div className="font-semibold">{desc.label}</div>
                <div className="text-[10px] mt-1" style={{ color: "var(--muted-foreground)" }}>{desc.type}</div>
              </div>
            </div>
          ))
        )}
