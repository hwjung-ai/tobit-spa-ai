...
import { API_TOUR_CONFIG, API_QUICK_TOUR_CONFIG } from "./apiTourConfig";
import { cn } from "@/lib/utils";

interface ApiTourLauncherProps {
...
/**
 * API Manager Tour Launcher Component
 *
 * Provides buttons to start the API Manager onboarding tour.
 * Automatically shows the tour for first-time users.
 */
export function ApiTourLauncher({
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      ...
      <button
        ...
        type="button"
        aria-label="Start full API Manager tour"
      >
        <svg
      ...
      <button
        ...
        type="button"
        aria-label="Start quick API Manager tour"
      >
        <svg
          className="h-3.5 w-3.5"
          fill="none"
 ...
 */
export function ApiTourIconButton({
  className,
  onTourStart,
  onTourComplete,
  onTourSkip,
}: Omit<ApiTourLauncherProps, "fullTourText" | "quickTourText">) {
  const {
    isActive,
    start,
    complete,
  ...
  return (
    <>
      <button
        ...
        type="button"
        aria-label="Start API Manager tour"
        title="Start Tour"
      >
        <svg
          className="h-4 w-4"
...
export default ApiTourLauncher;
